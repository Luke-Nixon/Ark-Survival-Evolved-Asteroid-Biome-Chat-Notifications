...
//				if (!get_value(i, v)) throw mysqlpp_exception(mysqlpp_exception::empty_result);
		...
		public:
			...
			bool open(const connect_options& options) {
				...
				if (options.ssl_mode >= 0) {
					my_bool b = (options.ssl_mode == 1) ? 0 : 1; 
					mysql_options(my_conn, MYSQL_OPT_SSL_ENFORCE, &b);
					mysql_options(my_conn, MYSQL_OPT_SSL_VERIFY_SERVER_CERT, &b);
					if (options.ssl_mode == 1) mysql_ssl_set(my_conn, nullptr, nullptr, nullptr, nullptr, nullptr);
				}
